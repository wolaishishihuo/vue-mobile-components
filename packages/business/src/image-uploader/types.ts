...
export interface ImgUploaderEmits {
  'update:modelValue': (value: (string | UploaderFileListItem)[]) => void;
  'change': (value: (string | UploaderFileListItem)[]) => void;
  'delete': (file: UploaderFileListItem, index: number) => void;
  'clickPreview': (file: UploaderFileListItem, index: number) => void;
  'beforeUpload': (file: File) => boolean | Promise<boolean>;
  'afterUpload': (file: UploaderFileListItem) => void;
  'oversize': (files: File[]) => void;
}

// 保持向后兼容
export interface UploaderProps extends ImgUploaderProps {}
